 ...
 */
package org.eighttwosixvalencia.tutoring.ejb;

import org.eighttwosixvalencia.tutoring.entity.*;
import org.eighttwosixvalencia.tutoring.util.CalendarUtil;
import org.eighttwosixvalencia.tutoring.util.StatusType;
...
public class RequestBean {
    ...
    private static final Logger logger = Logger.getLogger("org.eighttwosixvalencia.tutoring.ejb.RequestBean");
    ...
    public String checkOut(Student student) {
        ...
        // get today's session from the convenience method
        TutoringSession tutoringSession = this.getTodaysSession();
        logger.log(Level.INFO, "Setting {0}''s status to HOME", student.getFirstName());
        student.setStatus(StatusType.HOME);
        logger.log(Level.INFO, "Student {0} is {1}", new Object[]{student.getFirstName(), student.getStatus()});
        StatusEntry entry = new StatusEntry(StatusType.HOME, student, tutoringSession);
        // add the status entry to the tutoring session
        tutoringSession.getStatusEntries().add(entry);
        // persist the status entry
        logger.info("Persisting status entry");
    ...
    public void checkOutAllStudents() {
        ...
        Iterator<Student> j = allStudents.iterator();
        while (j.hasNext()) {
